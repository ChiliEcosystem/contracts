...
// Chili Ecosystem with Governance.
contract ChiliEcosystem is BEP20 {
    ...
    constructor() BEP20("ChiliEcosystem", "CHILI"){
    ...
    */
        ...
        external
    {
        ...
        require(signatory != address(0), "CHILI::delegateBySig: invalid signature");
        require(nonce == nonces[signatory]++, "CHILI::delegateBySig: invalid nonce");
        require(block.timestamp <= expiry, "CHILI::delegateBySig: signature expired");
    ...
    function getPriorVotes(address account, uint blockNumber)
        ...
        returns (uint256)
    {
        require(blockNumber < block.number, "CHILI::getPriorVotes: not yet determined");
...
contract MasterChef is Ownable, ReentrancyGuard {
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of CHILI
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accChiliPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accChiliPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 accChiliPerShare;   // Accumulated CHILISs per share, times 1e12. See below.
    }

    // The CHILI ECOSYSTEM TOKEN!
    ChiliEcosystem public chili;
    // Dev address.
    ...
    event UpdateStartTime(address indexed user, uint256 startBlock);
    event UpdateDevFundRate(address indexed user, uint256 Dev_Fund_Rate);

    constructor(
        ChiliEcosystem _chili,
        address _devaddr,
    ...
    function pendingChili(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 chiliReward = multiplier.mul(chiliPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accChiliPerShare = accChiliPerShare.add(chiliReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accChiliPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 chiliReward = multiplier.mul(chiliPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        chili.mint(devaddr, chiliReward.div(DevFundRate));
        chili.mint(address(this), chiliReward);
        pool.accChiliPerShare = pool.accChiliPerShare.add(chiliReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        updatePool(_pid);
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accChiliPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    function safeChiliTransfer(address _to, uint256 _amount) internal {
        ...
        if (_amount > chiliBal) {
            transferSuccess = chili.transfer(_to, chiliBal);
        } else {
            transferSuccess = chili.transfer(_to, _amount);
        }
        require(transferSuccess, "safeChiliTransfer: transfer failed");
    }
    ...
    function updateEmissionRate(uint256 _chiliPerBlock) public onlyOwner {
        massUpdatePools();
        chiliPerBlock = _chiliPerBlock;
        emit UpdateEmissionRate(msg.sender, _chiliPerBlock);
    }

    // Update fund dev rate.
    function updateDevFundRate(uint256 _DevFundRate) public onlyOwner {
        DevFundRate = _DevFundRate;
        emit UpdateDevFundRate(msg.sender, _DevFundRate);
    }

    // Update start time.
    function updateStartTime(uint256 _startBlock) public onlyOwner {
        startBlock = _startBlock;
        emit UpdateStartTime(msg.sender, _startBlock);
    }
}
